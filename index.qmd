...
**Answer:** 
Zip code should be modelled as a categorical variable.  Zip code is a non-ordinal variable because there is no inherent order to the zip codes.  The zip codes are simply a way to identify a geographic area and have no inherent relationship to house prices.
...
**Answer:** 
R does a better job of modelling zip code as a categorical variable because R has a built-in function for one-hot encoding.  Python does not have a built-in function for one-hot encoding, so you have to create your own function.  The documentation at [https://scikit-learn.org/stable/modules/tree.html](https://scikit-learn.org/stable/modules/tree.html) suggests a weakness in the Python implementation because the Python implementation does not handle categorical variables as well as the R implementation.  The R implementation uses the `rpart` package to model the zip code as a categorical variable.  The `rpart` package is a decision tree implementation that handles categorical variables better than the Python implementation.
...
**Answer:** 
Scikit-learn	OneHotEncoder is a good way to implement decision trees in Python with better categorical handling.  The OneHotEncoder is a scikit-learn transformer that converts categorical variables into a format that can be used by the decision tree.  The OneHotEncoder is a good way to implement decision trees in Python with better categorical handling.
